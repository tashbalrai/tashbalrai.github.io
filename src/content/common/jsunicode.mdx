...
import TitleBar from "../../components/Body/TitleBar.tsx";

# {frontmatter.title}

<TitleBar readtime="8" publishedAt={frontmatter.publishedAt} />
JavaScript uses the unicode to support the source code. It uses UTF-16 encoding for
handling text (code) in two ways, first one while parsing the source code internally
and second when a JavaScript code is loaded externally from a file for example loading
of the JavaScript file from http within a HTML page.
